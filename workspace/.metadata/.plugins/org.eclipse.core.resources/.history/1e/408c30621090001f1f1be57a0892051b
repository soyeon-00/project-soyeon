  ...
  document.querySelector('.next-button').addEventListener('click', () => {
    ...
    if (selectedUser) {
      ...
      if (selectedUser.value === 'A') {
        window.location.href = 'http://localhost:9000/powpow/buyer-join.jsp'; // POWPOW 회원 페이지
      } else if (selectedUser.value === 'B') {
        window.location.href = 'http://localhost:9000/powpow/seller-join.jsp'; // POWPOW 판매자 페이지
      }
    } else {
